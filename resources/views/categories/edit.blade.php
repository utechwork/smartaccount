        ...
        <form method="POST" action="{{ route('categories.update', $category) }}" class="space-y-6">
            ...
            <div>
                ...
                <div class="flex gap-4 items-center">
                    <input type="color" id="color" name="color" value="{{ old('color', $category->color ?? '#52C966') }}" 
                        class="h-12 border border-gray-300 rounded-lg cursor-pointer">
                    <input type="text" id="colorHex" value="{{ old('color', $category->color ?? '#52C966') }}" readonly
            ...
            <div class="bg-blue-50 p-4 rounded-lg">
                <p class="text-sm text-blue-900"><strong>Info:</strong></p>
                <p class="text-sm text-blue-800 mt-1">
                    This category is used in <strong>{{ $category->vendors->count() }}</strong> vendor(s) and 
                    <strong>{{ $category->accountStatements->count() }}</strong> transaction(s).
                </p>
            </div>
